...
function Analytics() {
  ...
  const topRisky = [...borrowers]
    .map((b) => ({ ...b, exposure: Math.max(totalDue(b) - b.paid, 0) }))
          ...
          <div className="mt-4 h-64">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={topRisky} layout="vertical">
                ...
                <YAxis type="category" dataKey="name" stroke="oklch(0.70 0.02 250)" fontSize={12} width={110} />
                <Tooltip contentStyle={{ background: "oklch(0.20 0.03 250)", border: "1px solid oklch(0.30 0.03 252)", borderRadius: 12 }} formatter={(v: number) => fmtINR(v)} />
            ...
            {topRisky.map((b) => (
              <li key={b.id} className="flex items-center justify-between py-3">
                <div>
                  ...
                  <div className="text-xs text-muted-foreground">{b.id} · {b.status}</div>
                </div>
                <div className="text-right">
                  <div className="font-medium">{fmtINR(b.exposure)}</div>
